        ...
        switch (type) {
            case 'Point':
                ...
                if (symbol === 'circle') {
                    ...
                    ////设置渐变  liwei
                    var radial = data.radial;
                    var gradient;
                    try{
                    	if(radial&&coordinates[0]&& coordinates[1]){
                            gradient = context.createRadialGradient(coordinates[0], coordinates[1], 0, coordinates[0], coordinates[1], size);
                            for(var nn in radial){
                            	gradient.addColorStop(nn, radial[nn]);
                            }
                    ...
                    var radial = data.radial;
                    if(radial){
                        var gradient = context.createRadialGradient(coordinates[0], coordinates[1], 0, coordinates[0], coordinates[1], size);
                        gradient.addColorStop(0, radial["0"]);
                        gradient.addColorStop(1, radial["1"]);
                        $.fillStyle = gradient;
                    }
                    $.fillStyle = gradient;
                    $.beginPath();
                    $.moveTo(0,-size);
                    $.quadraticCurveTo(0,0,-(size/3),0);
                    $.quadraticCurveTo(0,0,0,size);
                    $.quadraticCurveTo(0,0,(size/3),0);
                    $.quadraticCurveTo(0,0,0,-size);
                    $.closePath();
                    $.fill();
