...
window.BMAP_AUTHENTIC_KEY = myak;
//liwei
window.BMAP_STYLE_SEQ={};  var _e=0,_g=0,_c=0;
(function() {
	function aa(a) {
		throw a;
	}
	...
	D.U_ = D.verify || function() {
		D.version && D.version >= 1.5 && true/*pa(D.nd + "?qt=verify&ak=" + qa, function(a) {
			if (a && a.error !== 0) {
	...
	z.extend(Oa.prototype, {
		...
		IZ: function(a) {
			function b(a) {
				...
				//liwei
				//加载自定义样式
				var styleName = mapStyle+"/base.style";
				if(environment=="dev"){
					$.ajax({
						url: "livGis/downloadGisStyle",
						data:  { httpurl: b, name: styleName},
						type: "post",
						async:false,
						//dataType: "json",
				...
				b= "livGis/findGisStyle?name="+styleName;
				//liwei
				pa(b)
			}
			var c = this;
...
//				f = (1E5 * Math.random()).toFixed(0);
		...
		PW: function(a, b) {
			...
			//liwei 样式自定义 callback
			liwei = a+"_"+b.styleId;
			
			e = (1E5 * Math.random()).toFixed(0);
			window["_cbk" + liwei] = function(b) {
				b = 3 === b.data.style.length ? b.data.style[2] : b.data.style;
				c.UN(b, a);
				c.cF(a);
				b = new Q("onzoomfeatureload" + a);
				c.dispatchEvent(b);
				delete window["_cbk" + liwei]
			};
			var f = D.nd + "custom/v2/mapstyle?ak=" + qa + "&callback=_cbk" + liwei + "&",
			...
			//liwei
			//加载自定义样式
			var styleName = mapStyle+"/"+a+"_"+b.styleId+"_base.style";
			if(environment=="dev"){
				$.ajax({
					url: "livGis/downloadGisStyle",
					data:  { httpurl: f, name: styleName},
					type: "post",
					async:false,
					//dataType: "json",
					success: function(data){
			...
			f= "livGis/findGisStyle?name="+styleName;
			//liwei
		...
		if (a) {
			...
			var f = function(a) {
					a && (qb = p, setTimeout(function() {
						...
						// liwei 
						rb.src = "livGis/findGisPng?name=blank.gif";
					}, 50))
				},
				g = function() {
					var a = sb.shift();
	...
	// console.log("D.oa = "+D.url.proto + ("2" == D.Qt ? D.url.domain.main_domain_cdn.other[0] : D.url.domain.main_domain_cdn.baidu[0]) + "/");
	D.oa = "";
	...
	function(a, b, c) {
		...
		function(a, b, c) {
			...
			var m = Math.random,
				...
				k="livGis/findGisPng?name=st.gif";
				n = {
					kh: "0.1"
				},
				o = {
...
//			liwei
		...
		setTimeout(function() {
			...
			var filename = "";
			//图标请求js
			if(e.src.indexOf("bdimg.com")!=-1&&e.src.indexOf("getPoiData")==-1){
				// console.log("bdimg.com: "+src);
				filename = mapStyle+"/"+src.substring(src.lastIndexOf('/')+1,src.indexOf('.js?'))+".js";
			//主题及授权认证
			}else if(e.src.indexOf("business_accredit")!=-1||e.src.indexOf("custom_map")!=-1){
				filename =mapStyle+"/"+src.substring(src.indexOf('=')+1,src.indexOf('&'))+".js";
			//矢量瓦片
			}else if(e.src.indexOf("baidu.com")==-1){
				document.getElementsByTagName("head")[0].appendChild(e);

				//路径坐标
			}
			else if(getQueryVariableByUrl(src,"output")=="json"&&getQueryVariableByUrl(src,"qt")=="drct") {
				document.getElementsByTagName("head")[0].appendChild(e);
				console.log("111"+e.src);
					let ffilename = getQueryVariableByUrl(src,"mode")+getQueryVariableByUrl(src,"origin")+","+getQueryVariableByUrl(src,"destination");
					ffilename = ffilename.replace(/%2C/g,",")
					if(environment=="dev"){
						$.ajax({
							url: "livGis/saveRoadPoints",
							data:  { httpurl: src, name: ffilename},
							type: "post",
							async:false,
							//dataType: "json",
							success: function(data){
								console.log("下载路径坐标："+ffilename);
							}
						});
					}
				...
				{
//					filename =src.substring(src.indexOf('=')+1,src.indexOf('&'));
//					filename = mapStyle+"/"+filename+"_file.file";
				console.log("222"+e.src);
					document.getElementsByTagName("head")[0].appendChild(e);
			...
			if(filename){
				if(environment=="dev"){
					$.ajax({
						url: "livGis/downloadGisStyle",
						data:  { httpurl: src, name: filename},
						type: "post",
						async:false,
						//dataType: "json",
						success: function(data){
				...
				e.src = "livGis/findGisStyle?name="+filename;
				document.getElementsByTagName("head")[0].appendChild(e);
			}
			e = s
		}, 1)
			...
			if (e.Ke == this.tj.Dp) c && b();
				...
				if (e.Ke == this.tj.FF) {
					...
					f.PB == t && (f.PB = p, setTimeout(function() {
						...
						0 == a.length ? f.aK() : pa("js/getModules3.0.min.js")
						console.log("xx="+a);
						//debugger
					}, 1));
					e.Ke = this.tj.UO
				...
				for (var c = s, c = "2" == D.Qt ? [D.url.proto + D.url.domain.main_domain_cdn.other[0] + "/"] : [D.url.proto + D.url.domain.main_domain_cdn.baidu[0] + "/", D.url.proto + D.url.domain.main_domain_cdn.baidu[1] + "/", D.url.proto + D.url.domain.main_domain_cdn.baidu[2] + "/"], e = 0, f; f = this.mm[e]; e++)
					if (f.TZ == p) {
						...
						f.getTilesUrl = function(b, e) {
							...
							var nURL = c[Math.abs(f + m) % c.length] + n;

//							var nname = "customimage/"+f+"/"+m+"/"+e+".png";
							var nname = mapStyle+"/"+f+"/"+m+"/"+e+".png";
							
							/** 开启下载  mapStyle  非 v2 **/
							if(environment=="dev"){
								$.ajax({
									url: "livGis/downloadGisPng",
									data:  { httpurl: nURL, name: nname},
									async:false,
									type: "post",
        							//dataType: "json",
									success: function(data){
										//nURL = "findgispng?name="+nname;
									}
								});
							}
							
							nURL = "livGis/findGisPng?name="+nname;
							//console.log(nURL);
							return nURL;
		...
		Xr: function(a, b, c, e, f) {
			...
			if ("lanche" !== g)
				if (gd.ax[g]) this.PJ(a, b, gd.ax[g], e, f);
					...
					//  mapStyleV2
					var src = c;
					var filename = mapStyle+"/img_icon/"+src.substring(src.lastIndexOf('/')+1,src.indexOf('.png?'))+".png";
					if(environment=="dev"){
						$.ajax({
							url: "livGis/downloadGisStyle",
							data:  { httpurl: src, name: filename},
					...
					i.src = "livGis/findGisStyle?name="+filename;
...
//		 debugger
		...
		//console.log ("mylog:"+w);
		//修改为已下载的本地样式文件
		var styleName = mapStyle+"/"+b+"/"+c+"/"+e+".style";
		/** 开启下载 **/
		if(environment=="dev"){
			$.ajax({
				url: "livGis/downloadGisStyle",
				data:  { httpurl: w, name: styleName},
				type: "post",
				async:false,
				//dataType: "json",
				success: function(data){
		...
		//自动下载
		}else if(environment=="autodownload"){
			if(downloadtimeout){
		...
		w= "livGis/findGisStyle?name="+styleName;
		/* js样式文件*/
		
		window[y] = function(a) {
			//liwei 捕获异常避免报错导致暂停下载
			try{
				clearTimeout(i[g].Ik);
			}catch(e){}
	...
	Hd.getTilesUrl = function(a, b, c) {
		...
		var nURL = (Fd[Math.abs(e + a) % Fd.length] + "?qt=vtile&x=" + (e + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b + "&styles=" + c + "&scaler=" + g + (6 == z.fa.ma ? "&color_dep=32&colors=50" : "") + "&udt=" + f).replace(/-(\d+)/gi, "M$1")
		...
		//liwei  本地瓦片
		/*var xlll=a.x,ylll=a.y;
		return "mapImages/"+nname;
		*/	
...
//			var g = (1E5 * Math.random()).toFixed(0);
			...
			g= BMAP_STYLE_SEQ["g_"+_g];
			if(!g){
				BMAP_STYLE_SEQ["g_"+_g]=g=(1E5 * Math.random()).toFixed(0);
			}
			_g++;
